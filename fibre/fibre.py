...
# from charm.toolbox.group import PairingGroup, GT
...
def fibre2(pre, ID1, ID2, msg, master_key, params, group):
    # # Procedures:
    # C1 = IBE(m, ID1)
    # rk12 = genRK(sk1, ID2)
    # C2 = RE(C1, rk12)
    ...
    print("IBE ciphered Text is: ", C1)
    # print(type(serialize(C1['c1']['C1'])))

    aa = objectToBytes(C1, group)
    print(len(aa))
    CC = bytesToObject(aa, group)
    print(CC)

    print("hi")
    # C1['c1']['C2'] = serialize(C1['c1']['C2'])

    m2 =pre.decrypt_jet(params, sk1, C1)
