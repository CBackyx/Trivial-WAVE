...
def dispatch_args(args):
    ...
    if action == "mke":
        ...
        cocks_pkg = CocksPKG(1024) # The pkg can be viewed as the root private key for attestation
        attest_sk = cocks_pkg
        ...
        print(attest)
        print(pickle.loads(attest))

        print(len(pickle.dumps(enc_attest)))
        
        # transactor.uploadCert(pickle.dumps(subject) + pickle.dumps(policy), pickle.dumps(enc_attest))
